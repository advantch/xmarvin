...
class MemoryProvider(BaseModel, abc.ABC):
...
class Memory(BaseModel):
    ...
    provider: MemoryProvider = Field(
        default_factory=lambda: extension_settings.global_context.get_default_memory_provider(),
        ...
        if v is None:
            raise ValueError(
                unwrap(
                    ...
                    Please note that if you are using Marvin for the first
                    time, this error is expected because Marvin does not include
...
def get_memory_provider(provider: str) -> MemoryProvider:
    ...
    if provider.startswith("chroma"):
        ...
        import marvin.extensions.agent_memory.providers.chroma as chroma_providers

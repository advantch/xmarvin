...
class LocalFileStorage(BaseFileStorage):
    ...
    async def sync_with_openai_thread_async(self, thread_id: str) -> List[dict]:
        ...
        for file_id in file_ids:
            ...
            metadata = await self.save_file_async(BytesIO(content), file_id, {"purpose": file.purpose})
    ...
    async def upload_to_openai_async(self, file_id: Union[str, UUID], purpose: str) -> dict:
        client = get_openai_client()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    return await self.save_file_async(BytesIO(content), file_id, {"url": url})
                else:
                    raise Exception(f"Failed to download file from {url}")

    @expose_sync_method('request_presigned_url')
    async def request_presigned_url_async(self, file_id: str, private: bool = True) -> dict:

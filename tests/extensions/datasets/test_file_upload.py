...
async def test_presigned_url(file_storage):
    ...
    with patch.object(
        file_storage, "request_presigned_url", new_callable=AsyncMock
    ) as mock_request_url:
        ...
        response = await file_storage.request_presigned_url_async(
            file_id, private=False
        )
        ...
        with patch("httpx.put") as mock_put:
            ...
            res = await file_storage.save_file_async(
                BytesIO(file_content),
                file_id,
                {"size": len(file_content), "content_type": content_type},
            )

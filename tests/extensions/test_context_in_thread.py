...
from marvin.extensions.types.run import StartRun
import uuid
...
def test_thread_runner(tenant_user, respx_mock):
    ...
    start_run_schema = StartRun(
        message=message,
        run_id=run_id,
        thread_id=thread_id,
        channel_id=tenant_user.channel_id,
    ...
    with run_context(start_run_schema) as (run, thread, context):
        assert str(run.id) == str(start_run_schema.run_id)
        assert str(thread.id) == str(start_run_schema.thread_id)
        ...
        assert str(in_run_context.tenant_id) == str(tenant_user.tenant.id)
        assert str(in_run_context.channel_id) == str(tenant_user.channel_id)
